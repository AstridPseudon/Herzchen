...
class AssessmentModule:
        ...
        *,
        ...
        for index, value in enumerate(criterion_specs):
            if isinstance(value, ResourceRef) or hasattr(value, "ref"):
                ...
                criterion_ref = ResourceRef(self.store.authority, CRITERION_KIND, criterion_id, "rev-1")
        ...
        with self.store.transaction() as tx:
            ...
            receipt = self.store.mutate(envelope, event_type="assessment.scope.declared", result_ref=ResourceRef(scope_ref.authority, scope_ref.kind, scope_ref.id, "rev-1"), before_refs=(parent_ref,), effects={"criterion_refs": tuple(criterion_refs), "protocol": protocol, "logical_parent": True}, stream=ASSESSMENT_STREAM, transaction=tx)
            if prior is None:
                for child_ref, child_payload in child_specs:
                    self.store.put_identity(child_ref, child_payload, version=1, transaction=tx)
                    self.store.put_reference(child_ref, transaction=tx)
    ...
    def select_candidate(
        ...
        *,
        ...
        candidate_identity = self.store.get_identity(candidate_ref)
    ...
    def assess(
        ...
        *,
        ...
        result_ref = ResourceRef(self.store.authority, RESULT_KIND, request_key)
        ...
        reservation_ref = ResourceRef(self.store.authority, "reservation", request_key + "-reservation")
        operation_key = request_key + "-invocation"
        invocation_ref = ResourceRef(self.store.authority, INVOCATION_KIND, request_key)
        ...
        with self.store.transaction() as tx:
            ...
            reservation_stable = ResourceRef(self.store.authority, "reservation", reservation_ref.id)
            ...
            self.store.put_identity(invocation_ref, invocation_payload, version=1, transaction=tx)
            self.store.put_reference(invocation_ref, transaction=tx)
    ...
    def create_correction(
        ...
        *,
        ...
        correction_ref = ResourceRef(self.store.authority, CORRECTION_KIND, key)
        ...
        with self.store.transaction() as tx:
            receipt = self.store.mutate(self._envelope("assessment.correction.create", correction_ref, payload, key, self._actor(actor), expected_version=0), event_type="assessment.correction.created", result_ref=ResourceRef(self.store.authority, CORRECTION_KIND, key, "rev-1"), before_refs=(current.ref, current.parent_obligation_ref), effects={"result_ref": current.ref, "finding_refs": refs, "parent_obligation_ref": current.parent_obligation_ref}, stream=ASSESSMENT_STREAM, transaction=tx)
            self.store.mutate(self._envelope("assessment.result.link-correction", current.ref, result_payload, key + "-result", self._actor(actor), expected_version=current.version, expected_revision=current.ref.revision), event_type="assessment.result.correction-linked", effects={"correction_ref": correction_ref, "parent_obligation_ref": current.parent_obligation_ref}, stream=ASSESSMENT_STREAM, transaction=tx)
    ...
    def accept(
        ...
        *,
        ...
        decision_ref = ResourceRef(self.store.authority, DECISION_KIND, key)
    ...
    def dispose_unknown(
        ...
        *,
        ...
        with self.store.transaction() as tx:
            receipt = self.store.mutate(self._envelope("assessment.unknown.disposition", decision_ref, payload, key, selected_actor, expected_version=0), event_type="assessment.unknown.disposed", before_refs=(current.ref, current.parent_obligation_ref, current.candidate_ref, current.criterion_ref), effects={"unknown": True, "disposition": value.value, "authority": authority}, stream=ASSESSMENT_STREAM, transaction=tx)
        return self.get_decision(receipt.result_ref or decision_ref)
    ...
    def record_loop_facts(
        ...
        *,
        ...
        target = ResourceRef(self.store.authority, LOOP_FACTS_KIND, key)
        ...
        with self.store.transaction() as tx:
            receipt = self.store.mutate(self._envelope("assessment.loop-facts.record", target, payload, key, self._actor(actor), expected_version=0), event_type="assessment.loop-facts.recorded", before_refs=(parent,), effects={"facts_only": True, "inference": None}, stream=ASSESSMENT_STREAM, transaction=tx)
    ...
    def list_decisions(self, *, parent_obligation: Any = None) -> Tuple[Decision, ...]:
        ...
        rows = self.store.connection.execute("SELECT * FROM identities WHERE authority = ? AND kind = ? ORDER BY id", (self.store.authority, DECISION_KIND)).fetchall()
    ...
    def _write_finding(self, tx: Any, result_ref: ResourceRef, parent_ref: ResourceRef, index: int, spec: Mapping[str, Any], verdict: Verdict) -> ResourceRef:
        ...
        finding_ref = ResourceRef(self.store.authority, FINDING_KIND, finding_id)
        ...
        self.store.put_identity(finding_ref, payload, version=1, transaction=tx)
        self.store.put_reference(finding_ref, transaction=tx)
        return finding_ref
    ...
    def _stable_local(self, value: Any, kind: str) -> ResourceRef:
        if isinstance(value, ResourceRef):
            if value.authority != self.store.authority or value.kind != kind or value.revision is not None:
                raise AssessmentError(f"{kind} target must be an unpinned local ResourceRef")
            return value
        if isinstance(value, str):
            return ResourceRef(self.store.authority, kind, _opaque(value, kind + " id"))
        raise AssessmentError(f"{kind} target must be an unpinned local ResourceRef")
