...
class ProjectSheet:
    ...
    def instantiate_new_template(
        ...
        if hasattr(template, "seed"):
            ...
            engine = TemplateEngine(self.store, graph=self.graph, actor=actor or self.actor)
    ...
    def _observed_project(self, project: WorkRecord) -> dict[str, Any]:
        ...
        result["receipts"] = [event.event_id for event in self.store.list_events(stream="work:" + project.id)]
        return result
    ...
    def _assignment_observations(self, task_ref: ResourceRef) -> dict[str, Any]:
        ...
        rows = self.store.connection.execute("SELECT authority, kind, id, current_revision FROM identities WHERE authority = ? AND kind IN (?, ?, ?, ?) ORDER BY kind, id", (self.store.authority, ASSIGNMENT_KIND, DISPATCH_KIND, RESULT_KIND, REPORT_KIND)).fetchall()
        assignment_ids = set()
        for row in rows:
    ...
    def _decision_context(self, task: WorkRecord) -> Mapping[str, Any]:
        ...
        rows = self.store.connection.execute("SELECT authority, kind, id, current_revision FROM identities WHERE authority = ? AND kind = ? ORDER BY id", (self.store.authority, DECISION_KIND)).fetchall()
        ...
        rows = self.store.connection.execute("SELECT authority, kind, id, current_revision FROM identities WHERE authority = ? AND kind = ? ORDER BY id", (self.store.authority, WAIT_KIND)).fetchall()
        for row in rows:
            identity = self.store.get_identity(ResourceRef(row["authority"], row["kind"], row["id"], row["current_revision"]))
    ...
    def _documents(self, project: WorkRecord, task_ids: set[str], selected: Optional[Sequence[Any]]) -> Tuple[Mapping[str, Any], ...]:
        ...
        output = []
        rows = self.store.connection.execute("SELECT authority, kind, id, current_revision FROM identities WHERE authority = ? AND kind = 'dat.content.association' ORDER BY id", (self.store.authority,)).fetchall()
        for row in rows:
            identity = self.store.get_identity(ResourceRef(row["authority"], row["kind"], row["id"], row["current_revision"]))
            if identity is None:
                continue
            ...
            if binding_ref is None or (wanted is not None and binding_ref.id not in wanted):
                continue
            document = self.store.get_identity(ResourceRef(binding_ref.authority, binding_ref.kind, binding_ref.id))
            ...
            if document is not None:
                ...
                if current is not None:
                    revision = self.store.get_identity(ResourceRef(current.authority, current.kind, current.id, current.revision))
