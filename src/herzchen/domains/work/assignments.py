...
class ResponsibilityAssignments:
    ...
    def assign(
        ...
        *,
        ...
        assignment_id = "assignment-" + hashlib.sha256((self.store.authority + ":" + request_key).encode()).hexdigest()[:28]
        ref = ResourceRef(self.store.authority, ASSIGNMENT_KIND, assignment_id)
        responsibility_ref = ResourceRef(self.store.authority, RESPONSIBILITY_KIND, assignment_id)
        payload = {
        ...
        with self.store.transaction() as tx:
            ...
            receipt = self.store.mutate(envelope, event_type="work.assignment.created", result_ref=ResourceRef(ref.authority, ref.kind, ref.id, "rev-1"), effects={"responsibility": responsibility_ref, "generation": 1}, stream="assignment:" + assignment_id, transaction=tx)
            if prior is None:
                self.store.put_identity(responsibility_ref, {"record_type": "work.responsibility", "assignment": ref, "role": role, "principal": _json_safe(principal)}, version=0, transaction=tx)
                self.store.put_reference(responsibility_ref, transaction=tx)
        return self.get(ref)
    ...
    def get(self, target: Any) -> ResponsibilityAssignment:
        ...
        identity = self.store.get_identity(ref) if ref is not None else None
    ...
    def reassign(
        ...
        *,
        ...
        with self.store.transaction() as tx:
            ...
            self.store.mutate(envelope, event_type="work.assignment.reassigned", effects={"from_generation": current.generation, "to_generation": next_generation, "history_length": len(history)}, stream="assignment:" + current.id, transaction=tx)
    ...
    def dispatch(
        ...
        ref = ResourceRef(self.store.authority, DISPATCH_KIND, dispatch_id)
    ...
    def list_observations(self, target: Any, *, kind: Optional[str] = None) -> Tuple[ObservationRecord, ...]:
        assignment = self.get(target)
        rows = self.store.connection.execute("SELECT * FROM identities WHERE authority = ? AND kind IN (?, ?) ORDER BY id", (self.store.authority, RESULT_KIND, REPORT_KIND)).fetchall()
        result = []
        for row in rows:
    ...
    def _append_observation(self, target: Any, value: Any, *, kind: str, expected_generation: Optional[int], logical_request_key: Optional[str], actor: Optional[AuthenticatedActor]) -> ObservationRecord:
        ...
        ref = ResourceRef(self.store.authority, identity_kind, ident)
        ...
        with self.store.transaction() as tx:
            ...
            self.store.mutate(envelope, event_type="work." + kind + ".appended", result_ref=ResourceRef(ref.authority, ref.kind, ref.id, "rev-1"), after_refs=(assignment.ref,), effects={"assignment": assignment.ref, "generation": assignment.generation, "append_only": True}, stream=kind + ":" + assignment.id, transaction=tx)
    ...
    def _pin_reference(self, value: Any) -> ResourceRef:
        ...
        if ref.revision is not None:
            retained = self.store.get_reference(ref)
            ...
            return ref
        current = self.store.get_identity(ref)
        if current is None:
    ...
    def _resolve_ref(self, target: Any) -> Optional[ResourceRef]:
        ...
        if hasattr(target, "ref") and hasattr(target, "kind"):
            return ResourceRef(self.store.authority, ASSIGNMENT_KIND, target.ref.id)
        ...
        if isinstance(target, Mapping):
            ...
            if target.get("id") is not None:
                return ResourceRef(self.store.authority, ASSIGNMENT_KIND, str(target["id"]))
        if isinstance(target, str):
            return ResourceRef(self.store.authority, ASSIGNMENT_KIND, target)
        return None
