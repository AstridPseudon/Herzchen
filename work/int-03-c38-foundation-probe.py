...
def main() -> None:
    ...
    try:
        ...
        engine=TemplateEngine(store,graph=graph,actor=actor); rendered=engine.render(blank_project_template()); blank=engine.instantiate('work.blank_project',logical_request_key='snew-template',actor=actor); blank_record=graph.get(blank.project.ref); lifecycle=AuthoringLifecycle(AuthoringSessionService(store)); checkout=root/'blank-checkout'; checkout.mkdir(); (checkout/'project.json').write_bytes(b'{}'); opened=lifecycle.open(blank.project.ref,actor,request_id='snew-open',target_kind='project-sheet',base_revision=blank_record.revision,initial_content=b'{}',pending=True); close=lifecycle.idle_close(opened,request_id='snew-idle-close',checkout_root=checkout,registered_files=['project.json'],handler=batches.lifecycle_handler(blank.project,authoring=lifecycle.service,handle=opened.handle,request_id='snew-domain'),inactivity_seconds=1,last_content_edit=0,now=10,quiesce=lambda:True,writer_check=lambda:True); blank_after=graph.get(blank.project.ref)
        checks['built_in_blank_and_untouched_close']=rendered.seed.get('tasks')==[] and blank_after.lifecycle.value=='pending' and blank_after.revision==blank_record.revision and not blank_after.payload.get('tasks') and not (checkout/'project.json').exists() and getattr(close,'status',None)=='closed_cleaned' and not any(e.event_type=='work.project-sheet.applied' and e.subject.id==blank.project.id for e in store.list_events()); details['built_in_blank_and_untouched_close']={'template':safe(rendered),'creation_receipts':[safe(x) for x in blank.receipts],'initial_content_bytes':2,'open':safe(opened),'close':safe(close),'before':safe(blank_record),'after':safe(blank_after)}
    finally: store.close()
